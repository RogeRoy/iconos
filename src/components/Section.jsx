...
const CHIP_BG = {
  ...
  hl:'#7b5800',note:'#611232',hr:'#888',url:'#1e5b4f',mail:'#880e4f',img:'#a57f2c',
}
...
const PH = {
  ul:    'Escribe el primer punto de la lista y usa los botones de arriba para agregar más...',
  ol:    'Escriba el primer paso y use los botones de lista numerada para agregar más...',
  'ul-ol':'Escriba las viñetas principales. Use "Anidar" para crear sub-listas dentro de cada punto.',
  ...
  return (
    <div className={styles.elementoCard}>
      <div className={styles.elementoHead}>
        <span className={styles.chip} style={{ background: CHIP_BG[elemento.tipo] || '#555' }}>
          {info.label}
        </span>
        <button className={styles.btnEliminar} onClick={() => onDeleteElemento(elemento.id)} title="Eliminar este elemento">✕</button>
      </div>
  ...
  return (
    <div className={styles.seccionCard}>
      <div className={styles.seccionHead} onClick={() => setAbierta(!abierta)}>
        ...
        <button className={styles.btnEliminarSeccion}
          onClick={(e) => { e.stopPropagation(); onDelete(section.id) }} title="Eliminar sección">🗑</button>
      </div>
      ...
      {abierta && (
        <div className={styles.seccionBody}>
          ...
          <div className={styles.campo}>
            <label>Clases CSS del contenedor<span className={styles.labelHint}> (ej: doc-section)</span></label>
