...
export default function TitleEditor({ contenido, align, font, cssClases, onChange }) {
  ...
  const handleCase = (modo) => {
    ...
    const res=(s!==e) ? value.slice(0,s)+applyCase(value.slice(s,e),modo)+value.slice(e) : applyCase(value,modo)
    notificar({contenido:res})
    requestAnimationFrame(()=>{ inp.focus(); inp.setSelectionRange(s,s+(e-s)) })
  }

  // previewStyle aplica alineación y fuente al div del preview
  const previewStyle = {
  ...
  return (
    <div className={styles.wrap}>
      ...
      <StyleSelector font={font} onFontChange={v=>notificar({font:v})}
        previewText={contenido||''} previewStyle={previewStyle}/>
    </div>
  )
}
