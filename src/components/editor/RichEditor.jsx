...
function ListBar({ editorRef }) {
  ...
  return (
    <div className={styles.listBar}>
      ...
      <button className={styles.tbtn} title="Agregar viñeta (•) — punto sin número"
      ...
      <button className={styles.tbtn} title="Agregar número (1. 2. 3.) — punto con número"
      ...
      <button className={`${styles.tbtn} ${styles.tbtnW}`} title="Anidar — mover punto hacia adentro para crear sub-lista"
        onMouseDown={e=>{e.preventDefault();cmd('indent')}}>
      ...
      <button className={`${styles.tbtn} ${styles.tbtnW}`} title="Quitar — sacar punto de la sub-lista hacia afuera"
        onMouseDown={e=>{e.preventDefault();cmd('outdent')}}>
...
export default function RichEditor({ tipo, html, align, font, cssClases, hasListBar=false, hasStyleRow=false, placeholder='Escribe aquí...', onChange }) {
  ...
  useEffect(()=>{ const ed=editorRef.current; if(ed) ed.innerHTML=html||'' },[]) // eslint-disable-line
  ...
  const notificar = (extras={}) => {
    ...
    const h=ed.innerHTML; setPreviewHtml(h)
    onChange({html:h,align:alignVal,font:fontCls,cssClases:[...new Set(parts)].join(' ')})
  }
  ...
  // clase visual del área editable — hl y note tienen estilo propio
  const edCls = tipo==='hl' ? styles.editorHl : tipo==='note' ? styles.editorNote : styles.editor

  // previewStyle: alineación actual + padding-left para listas
  const previewStyle = {
    textAlign: align||'left',
    paddingLeft: ['ul','ol','ul-ol'].includes(tipo) ? '28px' : undefined,
  }
    ...
    <div className={styles.richWrap}>
      ...
      <div className={styles.toolbar}>
        <div className={styles.grupo}>
          {[
            {cmd:'bold',       title:'Negrita — hace el texto más grueso', icon:<svg viewBox="0 0 20 20" width="15" height="15"><text x="4" y="15" fontFamily="Georgia,serif" fontSize="13" fontWeight="900" fill="#3a0a0a">B</text></svg>},
            ...
            {cmd:'underline',  title:'Subrayado — línea debajo del texto',  icon:<svg viewBox="0 0 20 20" width="15" height="15"><text x="4" y="13" fontFamily="Arial" fontSize="12" fontWeight="700" fill="currentColor">U</text><line x1="3" y1="16" x2="15" y2="16" stroke="currentColor" strokeWidth="2" strokeLinecap="round"/></svg>},
            {cmd:'hiliteColor',value:'yellow',title:'Resaltar — fondo amarillo',xc:styles.btnHl,icon:<svg viewBox="0 0 20 20" width="15" height="15"><rect x="3" y="11" width="14" height="5" rx="1" fill="#ffe566" opacity=".9"/><text x="4" y="13" fontFamily="Arial" fontSize="10" fontWeight="700" fill="#7b5800">Aa</text></svg>},
          ].map(({cmd,value,title,icon,xc})=>(
            <button key={cmd} className={`${styles.tbtn} ${xc||''} ${activeFormats[cmd]?styles.active:''}`}
              title={title} onMouseDown={e=>{e.preventDefault();handleFormat(cmd,value)}}>{icon}</button>
          ))}
          <div className={styles.sep}/>
        </div>
        <div className={styles.grupo}>
      ...
      {/* Área editable — estilo visual cambia según tipo */}
      <div ref={editorRef} className={edCls}
        style={{ textAlign:align||'left', fontFamily:FONT_FAMILY[font]||'inherit', minHeight:minH }}
      ...
      {hasStyleRow && (
        <StyleSelector font={font} onFontChange={v=>notificar({font:v})}
          previewHtml={previewHtml} previewStyle={previewStyle}/>
      )}
    </div>
  )
}
