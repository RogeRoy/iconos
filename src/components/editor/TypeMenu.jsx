// TypeMenu.jsx
// Menú acordeón de 4 grupos para agregar elementos a una sección.
// Solo un grupo puede estar abierto a la vez (igual que el HTML original).
// UX para +50 años: etiquetas cortas + descripción larga visible, iconos grandes,
// botones amplios, descripción al hover/tap.

import { useState } from 'react'
import { TYPE_GROUPS } from './elementTypes'
import styles from './TypeMenu.module.css'

// SVG icons — mismo diseño que el HTML original
function TypeIcon({ tipo }) {
  ...
  switch (tipo) {
    ...
    case 'h3': return <svg viewBox="0 0 22 22" fill="none" width="22" height="22"><text x="3" y="15" fontFamily="Arial,sans-serif" fontSize="10" fontWeight="700" fill={vd}>T</text><rect x="1" y="17" width="20" height=".9" rx=".45" fill={vd} opacity=".4"/></svg>
...
export default function TypeMenu({ seccionId, onAgregar }) {
  // Solo un grupo abierto a la vez — null = todos cerrados
  const [grupoAbierto, setGrupoAbierto] = useState(null)

  const toggleGrupo = (idx) => {
    // Si ya estaba abierto, lo cierra. Si estaba cerrado, lo abre y cierra los demás.
    setGrupoAbierto((prev) => (prev === idx ? null : idx))
  }

  const handleAgregar = (tipo) => {
    onAgregar(seccionId, tipo)
    // Cerrar todos los grupos al agregar (igual que el HTML original)
        ...
        {TYPE_GROUPS.map((grupo, gi) => {
          ...
          return (
            <div key={gi} className={styles.grupoAcc}>
              {/* Cabecera del grupo — clic abre/cierra */}
              <button
                className={`${styles.grupoHeader} ${abierto ? styles.grupoHeaderAbierto : ''}`}
              ...
              {/* Cuerpo del grupo — visible solo si está abierto */}
              {abierto && (
                  ...
                  <div className={styles.tiposBtns}>
                    {grupo.types.map((tipo) => (
                      <button
                        ...
                        onClick={() => handleAgregar(tipo.key)}
                        title={tipo.descLarga}
                      >
                        <span className={styles.tipoBtnIcono}>
                          <TypeIcon tipo={tipo.key} />
                        </span>
